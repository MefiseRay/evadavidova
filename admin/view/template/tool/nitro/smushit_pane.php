    ...
    <div class="col-md-8">
        ...
        <div class="box-content">
            <div class="" style="margin-bottom:15px;">
                <div class="col-md-8">
                    <div class="box-minibox">Optimized Images
                        <div class="number"
                             id="smushedNumber"><?php echo !empty($smushit_data['smushed_images_count']) ? $smushit_data['smushed_images_count'] : 0; ?></div>
                    </div>
                    <div class="box-minibox">Already Optimized
                        <div class="number"
                             id="alreadySmushedNumber"><?php echo !empty($smushit_data['already_smushed_images_count']) ? $smushit_data['already_smushed_images_count'] : 'N/A'; ?></div>
                    </div>
                    <div class="box-minibox">Total Images
                        <div class="number"
                             id="totalImages"><?php echo !empty($smushit_data['total_images']) ? $smushit_data['total_images'] : 'N/A'; ?></div>
                    </div>
                    <div class="box-minibox">KB saved
                        <div class="number"
                             id="kbSaved"><?php echo !empty($smushit_data['kb_saved']) ? $smushit_data['kb_saved'] : 0; ?>
                            KB
                        </div>
                    </div>
                    <div class="box-minibox">Last optimization
                        <div class="number"
                             id="lastSmushTimestamp"><?php echo !empty($smushit_data['last_smush_timestamp']) ? date('D, j M Y H:i:s', $smushit_data['last_smush_timestamp']) : 'N/A'; ?></div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="progress" style="margin-top: 10px; text-align: center;">
                        <div id="progressBar" class="progress-bar" style="width: 0%;line-height: 20px;color:#000;">0%
                        </div>
                    </div>
                </div>
            ...
            <button type="button" class="btn btn-large btn-primary smushItButton">Start new optimization process
            </button>
            <button type="button" class="btn btn-default btn-large resumeSmushButton" style="display: none">Resume
                previous optimization
            </button>
            <button type="button" class="btn btn-large btn-default pauseSmushButton" style="display: none">Pause
            </button>
            ...
            <table class="form">
                <tr>
                    <td style="vertical-align:top;" colspan="2">
                        <button class="btn btn-success hidden" id="optimizeReportedImagesButton">Optimize reported
                            images
                        </button>
                    </td>
                </tr>
                <tr>
                    ...
                    <td style="vertical-align:top;">
                        <select class="form-control" name="Nitro[Smush][Method]" id="smushMethod">
                            <option value="local" <?php echo ((empty($nitroData['Nitro']['Smush']['Method']) || $nitroData['Nitro']['Smush']['Method'] == 'local')) ? 'selected=selected' : '' ?>>
                                Local
                            </option>
                            <option value="remote" <?php echo (!empty($nitroData['Nitro']['Smush']['Method']) && $nitroData['Nitro']['Smush']['Method'] == 'remote') ? 'selected=selected' : '' ?>>
                                Remote
                            </option>
                        </select>
                    </td>
                ...
                <tr>
                    <td style="vertical-align:top;">Optimization Type<span class="help">Choose between Lossy or Lossless compression. Lossy compression will reduce the image quality, but result in a higher PageSpeed score. Lossless compression will preserve the image quality but will result in a lower PageSpeed score.</span>
                    </td>
                    <td style="vertical-align:top;">
                        <select class="form-control" name="Nitro[Smush][Type]" id="smushType">
                            <option value="lossy" <?php echo ((!empty($nitroData['Nitro']['Smush']['Type']) && $nitroData['Nitro']['Smush']['Type'] == 'lossy')) ? 'selected=selected' : '' ?>>
                                Lossy (lower quality, better PageSpeed score)
                            </option>
                            <option value="lossless" <?php echo (empty($nitroData['Nitro']['Smush']['Type']) || $nitroData['Nitro']['Smush']['Type'] == 'lossless') ? 'selected=selected' : '' ?>>
                                Lossless (better quality, lower PageSpeed score)
                            </option>
                        </select>
                    </td>
                </tr>
                <tr style="display: none" id="smushQuality">
                    ...
                    <td style="vertical-align:top;">
                        <input class="form-control" id="smushQualityInput" type="number" name="Nitro[Smush][Quality]"
                               value="<?php echo !empty($nitroData['Nitro']['Smush']['Quality']) ? $nitroData['Nitro']['Smush']['Quality'] : '80' ?>"
                               min="1" max="99">
                    </td>
                </tr>
                <tr style="display: none" id="smushOnDemand">
                    <td style="vertical-align:top;">Optimize On-The-Fly<span class="help">If enabled, your images will be optimized on-the-fly, while their cached version is created. Your first-time page load when the cache is being created may be slightly slower.</span>
                    </td>
                    <td style="vertical-align:top;">
                        <select class="form-control" name="Nitro[Smush][OnDemand]">
                            <option value="yes" <?php echo ((!empty($nitroData['Nitro']['Smush']['OnDemand']) && $nitroData['Nitro']['Smush']['OnDemand'] == 'yes')) ? 'selected=selected' : '' ?>>
                                Enabled
                            </option>
                            <option value="no" <?php echo (empty($nitroData['Nitro']['Smush']['OnDemand']) || $nitroData['Nitro']['Smush']['OnDemand'] == 'no') ? 'selected=selected' : '' ?>>
                                Disabled
                            </option>
                        </select>
                    </td>
                ...
                <tr>
                    <td style="vertical-align:top;">Optimize directory/file<span class="help">Enter path to a specific directory or a single file to be optimized. The path should be relative to the root of your OpenCart installation. Use "/" for directory separator even if your server is running on Windows.<br/><strong>Warning:</strong> The optimization process will overwrite the original images with the optimized ones, so a backup is recommended!</span>
                    </td>
                    <td style="vertical-align:top;">
                        <input class="form-control" id="smushTargetPath" type="text" name="Nitro[Smush][target_path]"
                               value="<?php echo !empty($nitroData['Nitro']['Smush']['target_path']) ? $nitroData['Nitro']['Smush']['target_path'] : '' ?>"
                               placeholder="image/cache/">
                    </td>
                </tr>
                <tr>
                    <td style="vertical-align:top;">Remove Image Cookies<span class="help">Enable this to speed up the delivery of your website images.</span>
                    </td>
                    <td style="vertical-align:top;">
                        <select name="Nitro[ImageCookies][Enabled]" class="form-control NitroImageCookiesEnabled">
                            <option value="yes" <?php echo ((!empty($nitroData['Nitro']['ImageCookies']['Enabled']) && $nitroData['Nitro']['ImageCookies']['Enabled'] == 'yes')) ? 'selected=selected' : '' ?>>
                                Yes (Recommended)
                            </option>
                            <option value="no" <?php echo (empty($nitroData['Nitro']['ImageCookies']['Enabled']) || $nitroData['Nitro']['ImageCookies']['Enabled'] == 'no') ? 'selected=selected' : '' ?>>
                                No
                            </option>
                        </select>
                    </td>
