...
<div id="content">
  ...
  <div class="container-fluid"> <!-- continut -->
    ...
    <div class="panel panel-default"> <!-- updates div -->
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-pencil"></i> <?php echo $masstxt_p_filters_updates; ?></h3>
      </div>
      ...
      <div class="table-responsive">
    	...
    	<tr>
    	  ...
    	  <strong><?php echo $masstxt_new_filters; ?></strong>
    	  </td>
    	  <td class="text-left">
    	  <div class="well well-sm scroll1">
                  <?php foreach ($p_filters as $filter) { ?>
                  <div class="checkbox">
                  <label>
                    <?php if (in_array($filter['filter_id'], $filters_ids_upd)) { ?>
                    <input type="checkbox" name="filters_ids_upd[]" value="<?php echo $filter['filter_id']; ?>" checked="checked" />
                    <?php echo $filter['group'].' &gt; '.$filter['name']; ?>
                    <?php } else { ?>
                    <input type="checkbox" name="filters_ids_upd[]" value="<?php echo $filter['filter_id']; ?>" />
                    <?php echo $filter['group'].' &gt; '.$filter['name']; ?>
                    <?php } ?>
                  </label>
                  </div>
