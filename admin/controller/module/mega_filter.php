...
class ControllerModuleMegaFilter extends Controller
{
    ...
    private $_version = '2.0.5.5.1';

    private $_plusRequired = '1.2.8';
    ...
    public function __construct($registry)
    {
        ...
        if (!empty($this->request->get['reactivate_license'])) {
            $this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `key` = 'mfilter_license'");
            $this->config->set('mfilter_license', null);
        }
    }
    ...
    public function index()
    {
        ...
        $this->data['action_remove_item'] = $this->url->link('module/' . $this->_name . '/remove_item', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_find_values'] = $this->url->link('module/' . $this->_name . '/findValuesForSelectpicker', 'token=' . $this->session->data['token'], 'SSL');
    ...
    private function _init($tab)
    {
        ...
        // install/update
        if ((version_compare(VERSION, '2.3.0.0', '<') || $tab != 'license') && (!$curr_ver || version_compare($curr_ver, $this->_version, '<') || $this->_isOldMFilterPlus())) {
            $this->load->model('setting/setting');
            $this->load->model('setting/store');
        ...
        if (NULL != ($mlicense = $this->config->get('mfilter_license'))) {
            if (!file_exists(DIR_SYSTEM . 'library/mfilter_core.php') || strpos(file_get_contents(DIR_SYSTEM . 'library/mfilter_core.php'), 'MegaFilterCore') === false) {
                /* @var $warning string */
                $warning = 'Detected missing files - please <a href="' . $this->url->link('module/' . $this->_name, 'token=' . $this->session->data['token'] . '&reactivate=1&missing=1', 'SSL') . '">click here</a> to fix problem';

                if (!empty($this->request->get['missing'])) {
                    $warning = 'It seems that something went wrong. Please try to re-activate the license - <a href="' . $this->url->link('module/' . $this->_name, 'token=' . $this->session->data['token'] . '&reactivate_license=1', 'SSL') . '">click here</a>';
                }
                ...
                if (!empty($this->request->get['reactivate'])) {
                    ...
                    $this->response->redirect(
                        $this->url->link('module/' . $this->_name, 'token=' . $this->session->data['token'] . (empty($this->request->get['missing']) ? '' : '&missing=1'), 'SSL')
                    );
                }
    ...
    private function verifyLicense($tab)
    {
        ...
        if (!in_array($tab, array('license', 'installprogress', 'seo_base_settings')) && !$this->config->get('mfilter_license')) {
            $this->response->redirect($this->url->link('module/' . $this->_name . '/license', 'token=' . $this->session->data['token'], 'SSL'));
        }
    }
    ...
    protected function toSelect($type, $idx = 0, $q = null)
    {
        ...
        if ($type == 'attribs') {
            foreach ($this->model_module_mega_filter->getAttributes(array('start' => 0, 'limit' => 1000, 'filter_name' => $q)) as $attribute) {
                $list[] = array(
            ...
            foreach ($this->model_module_mega_filter->getOptions(array('start' => 0, 'limit' => 1000, 'filter_name' => $q)) as $option) {
                $list[] = array(
                    'label' => $option['name'],
                    'name' => $option['name'],
                    'id' => $option['option_id'],
            ...
            foreach ($this->model_module_mega_filter->getFilters(array('start' => 0, 'limit' => 1000, 'filter_name' => $q)) as $filter) {
                $list[] = array(
                    'label' => $filter['name'],
                    'name' => $filter['name'],
    ...
    public function ldv()
    {
        ...
        switch ($type) {
            ...
            case 'configuration':
                $this->data['configuration'] = $this->config->get($this->_name . '_settings');
                break;
            case 'vehicles'        :
                $this->data['vehicles'] = $this->config->get($this->_name . '_vehicles');
                break;
            case 'levels'        :
                $this->data['levels'] = $this->config->get($this->_name . '_levels');
                break;
        }
    ...
    public function seo()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->checkPermission()) {
            ...
            if (in_array($this->request->post[$this->_name . '_seo']['url_parameter'], array('filter', 'path', 'tag'))) {
