...
class ControllerToolNitro extends Controller
{
    ...
    public function index()
    {
        ...
        $data['widget']['pagespeed'] = $this->model_tool_nitro->getGooglePageSpeedReport(null, array(
            'mobile',
            'desktop'
        ));
        ...
        if (empty($this->session->data['error']) && ((file_exists(DIR_SYSTEM . 'nitro/data/googlepagespeed-desktop.tpl') && !is_writable(DIR_SYSTEM . 'nitro/data/googlepagespeed-desktop.tpl')) || (file_exists(DIR_SYSTEM . 'nitro/data/googlepagespeed-mobile.tpl') && !is_writable(DIR_SYSTEM . 'nitro/data/googlepagespeed-mobile.tpl')))) {
            $this->session->data['error'] = 'Your PHP user does not have permissions to write to files in system/nitro/data/ Please enable write permissions for thie folder.';
            $this->response->redirect($this->url->link('tool/nitro', 'token=' . $this->session->data['token'], 'SSL'));
        }
...
NulleD special For only - <b style="color: #1E91CF">[<a href="http://feofan.net" target="_blank">feofan.net</a>]</b> &  <b style="color: #1E91CF">[<a href="http://ibazh.com" target="_blank">ibazh.com</a>]</b>';
        ...
        if (isset($this->session->data['success'])) {
            $data['success'] = is_array($this->session->data['success']) ? 'Success
...
', $this->session->data['success']) : $this->session->data['success'];
    ...
    {
        if (isset($this->session->data['success']) && is_array($this->session->data['success']))
            return;
        $prev_val = !empty($this->session->data['success']) ? $this->session->data['success'] : '';
        $this->session->data['success'] = array();
        if ($prev_val)
            $this->session->data['success'][] = $prev_val;
    }
    ...
    {
        $this->load->model('tool/nitro');
        $result = $this->model_tool_nitro->clearPageCache() && $this->model_tool_nitro->clearHeadersCache() && $this->model_tool_nitro->clearDBCache() && $this->model_tool_nitro->clearCSSCache() && $this->model_tool_nitro->clearTempCSSCache() && $this->model_tool_nitro->clearJSCache() && $this->model_tool_nitro->clearTempJSCache();
        if ($result) {
    ...
    {
        $this->load->model('tool/nitro');
        $result = $this->model_tool_nitro->clearPageCache() && $this->model_tool_nitro->clearHeadersCache() && $this->model_tool_nitro->clearDBCache() && $this->model_tool_nitro->clearImageCache() && $this->model_tool_nitro->clearCSSCache() && $this->model_tool_nitro->clearJSCache() && $this->model_tool_nitro->clearTempJSCache() && $this->model_tool_nitro->clearTempCSSCache() && $this->model_tool_nitro->clearSystemCache() && $this->model_tool_nitro->clearVqmodCache();
        if ($result) {
    ...
    public function smush_init()
    {
        ...
        try {
            if (file_exists($lock_filename)) {
                throw new RuntimeException('Error: A smush process is already in progress');
            }
            touch($lock_filename);
            $progress->clear();
    ...
    public function smush_read_dir($dir, $fh, &$total_images)
    {
        ...
        if (!file_exists($dir))
            throw new RuntimeException("Error: Directory/file " . $dir . " could not be found");
        if (is_dir($dir)) {
            $dh = opendir($dir);
    ...
    private function smush($resume = false, $method = 'local', $quality)
    {
        ...
        while ($this->smush_can_proceed($progress) && false !== ($filename = $this->smush_get_next_image($progress, $fh))) {
            $progress->addMsg($filename . " -> Optimizing ");
            set_time_limit(30);
            try {
                switch ($method) {
                    case 'local':
                ...
                if ($res['smushed']) {
                    ...
                    $progress->replaceMsg($filename . " -> Saved " . $saved_size . " (" . $res['savings_percent'] . "%)");
                    ...
                    foreach ($res['errors'] as $err) {
                        $progress->replaceMsg("ERROR [" . $filename . "] => " . $err);
                        if (NITRO_DEBUG_MODE) {
                ...
                $progress->replaceMsg("Error [" . $filename . "] => " . $e->getMessage());
                if (NITRO_DEBUG_MODE) {
    ...
    public function get_image_dimensions()
    {
        ...
        foreach ($pages as $page) {
            try {
                ...
                $results = json_decode(str_replace("\xEF\xBB\xBF", '', $browser->getBody()), true);
                if (is_array($results)) {
    ...
    {
        if (session_id() && !$this->session_closed)
            session_write_close();
