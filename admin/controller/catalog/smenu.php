        ...
        if (isset($this->request->get['smenu_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            ...
            foreach ($root_items as $items) {
                ...
                foreach ($childs as $child) {
                    ...
                    $childs2 = $this->model_catalog_smenu->getItems($this->request->get['smenu_id'], $child['item_id']);
                    foreach ($childs2 as $child2) {
                        $children_data2[] = array(
                            'item_id' => $child2['item_id'],
                            'order' => $child2['order'],
                            'type' => $child2['type'],
                            'type_id' => $child2['type_id'],
                            'type_name' => $child2['type_name'],
                            'description' => $child2['smenu_item_description']
                        );
                    }
                    $children_data[] = array(
