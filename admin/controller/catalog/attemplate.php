<?php
class ControllerCatalogAttemplate extends Controller {
	private $error = array();
 
	public function index() {
		$this->language->load('catalog/attemplate');

		$this->document->setTitle($this->language->get('heading_title'));
		
		$this->load->model('catalog/attemplate');
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_attemplate->editAttemplate($this->request->get['attemplate_id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('catalog/attemplate', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_attemplate->addAttemplate($this->request->post);
	...
	public function delete() { 
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $attemplate_id) {
				$this->model_catalog_attemplate->deleteAttemplate($attemplate_id);
			}
	...
	protected function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/attemplate', 'token=' . $this->session->data['token'] . $url, 'SSL')
   		);
							
		$data['insert'] = $this->url->link('catalog/attemplate/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('catalog/attemplate/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');	
		...
		$attemplate_total = $this->model_catalog_attemplate->getTotalAttemplates($filter_data);
		
		$results = $this->model_catalog_attemplate->getAttemplates($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['attemplates'][] = array(
				...
				'edit'		=> $this->url->link('catalog/attemplate/update', 'token=' . $this->session->data['token'] . '&attemplate_id=' . $result['attemplate_id'] . $url, 'SSL')
			);
		}	
		...
		$data['sort_name'] = $this->url->link('catalog/attemplate', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');		
		$data['sort_status'] = $this->url->link('catalog/attemplate', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/attemplate', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('catalog/attemplate_list.tpl', $data));		
	}
	...
	protected function getForm() {
		...
		$data['text_allow_multiple'] = $this->language->get('text_allow_multiple');
		$data['text_preset_value'] = $this->language->get('text_preset_value');
		...
		$data['button_remove'] = $this->config->get('button_remove');
		$data['button_attribute_add'] = $this->config->get('button_attribute_add');
		...
		if (!isset($this->request->get['attemplate_id'])) { 
			$data['action'] = $this->url->link('catalog/attemplate/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/attemplate/update', 'token=' . $this->session->data['token'] . '&attemplate_id=' . $this->request->get['attemplate_id'] . $url, 'SSL');
		}
		
		$data['cancel'] = $this->url->link('catalog/attemplate', 'token=' . $this->session->data['token'] . $url, 'SSL');
		
		$data['attemplate_id'] ='0';
		
		if (isset($this->request->get['attemplate_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$attemplate_info = $this->model_catalog_attemplate->getAttemplate($this->request->get['attemplate_id']);
		...
		if (isset($this->request->post['attributes'])) {
			...
			$attributes = $this->model_catalog_attemplate->getAttemplateAttributes($this->request->get['attemplate_id']);
		} else {
			$attributes = array();
		}
		...
		foreach ($attributes as $attribute) {
			...
			if (count($attribute['preset_id']) == 1) {
				$this->load->model('atpresets/atpresets');
				$preset_id = $attribute['preset_id'];
				$attribute['preset'] = html_entity_decode($this->model_atpresets_atpresets->getPresetTitle($attribute['preset_id'][0]),ENT_QUOTES, 'UTF-8');
		...
		if ($data['atpresets_selecttype']==1) {
			...
			$results = $this->model_atpresets_atpresets->getPresetsAuto(array('attribute_id'=>''));
		...
		$this->response->setOutput($this->load->view('catalog/attemplate_form.tpl', $data));			
	}
	
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'catalog/attemplate')) {
	...
	public function update_attributes() {
		...
		if ($this->request->get['option'] != 0 && $this->request->get['attemplate_id'] != -1) {
			...
			foreach ($attributes as $attribute) {
				...
				$preset = array();
				$presets_text = $this->model_atpresets_atpresets->getManyPresetsTexts($preset_id);
			...
			foreach ($old_attributes as $old_attribute) {
				...
				if (count($old_attribute['preset_id']) == 1) {
					...
					$old_attribute['preset'] = html_entity_decode($this->model_atpresets_atpresets->getPresetTitle($old_attribute['preset_id'][0]),ENT_QUOTES, 'UTF-8');
			...
			foreach ($product_attributes as $product_attribute) {
				...
				if (count($product_attribute['preset_id']) == 1) {
					...
					$product_attribute['preset'] = html_entity_decode($this->model_atpresets_atpresets->getPresetTitle($product_attribute['preset_id'][0]),ENT_QUOTES, 'UTF-8');
	...
	public function save_attemplate() {
		...
		if (!isset($json['error'])) {
			...
			$attemplate_id = $this->model_catalog_attemplate->addAttemplate($data);
			if ($attemplate_id)
