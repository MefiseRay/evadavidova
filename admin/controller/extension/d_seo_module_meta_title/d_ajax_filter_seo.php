<?php
class ControllerExtensionDSEOModuleMetaTitleDAjaxFilterSEO extends Controller {
	private $codename = 'd_ajax_filter_seo';
	private $route = 'extension/d_seo_module_meta_title/d_ajax_filter_seo';
	...
	public function store_meta_elements_links($store_meta_elements) {	
		...
		foreach ($store_meta_elements as $store_id => $meta_elements) {
			foreach ($meta_elements as $meta_element_key => $meta_element) {
				if (strpos($meta_element['route'], 'af_query_id') === 0) {
					$route_arr = explode("af_query_id=", $meta_element['route']);
				
					if (isset($route_arr[1])) {
						$query_id = $route_arr[1];
						$store_meta_elements[$store_id][$meta_element_key]['link'] = $this->url->link('extension/d_ajax_filter_seo/query/edit', $url_token . '&query_id=' . $query_id, true);
					}
				} 
			}
		}
