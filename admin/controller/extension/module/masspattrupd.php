...
class ControllerExtensionModulemasspattrupd extends Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        $this->load->language('extension/module/masspattrupd');

        $this->document->addStyle('view/template/extension/module/masspattrupd.css');
        ...
        $data['masstxt_upd_mode_de_a_v'] = $this->language->get('masstxt_upd_mode_de_a_v');
        $data['masstxt_upd_mode_de_a_v_help'] = $this->language->get('masstxt_upd_mode_de_a_v_help');
        $data['masstxt_upd_mode_de_a'] = $this->language->get('masstxt_upd_mode_de_a');
        $data['masstxt_upd_mode_de_a_help'] = $this->language->get('masstxt_upd_mode_de_a_help');
        ...
        $data['masstxt_upd_mode_de_val'] = $this->language->get('masstxt_upd_mode_de_val');
        $data['masstxt_upd_mode_de_val_help'] = $this->language->get('masstxt_upd_mode_de_val_help');
        ...
        if (isset($this->request->post['mass_update'])) { /// update button
            ...
            if ($this->user->hasPermission('modify', 'extension/module/masspattrupd')) { /// modify permision

                if (isset($this->request->post['selected'])) { /// avem produse selectate

                    if ($this->request->post['upd_mode'] != '') { /// avem attribute update
                        ...
                        foreach ($this->request->post['selected'] as $product_id) { /// scanare produse
                            ...
                            switch ($this->request->post['upd_mode']) { /// update mode
                                ...
                                case "de_a_v": // Remove EXISTING attributes and EXISTING values that exactly match with SELECTED attributes and SELECTED values
                                    if (!empty($this->request->post['product_attribute'])) {
                                        foreach ($this->request->post['product_attribute'] as $product_attribute) {
                                            if ($product_attribute['attribute_id']) {
                                                foreach ($product_attribute['product_attribute_description'] as $language_id => $product_attribute_description) {
                                                    $this->db->query("DELETE FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "' AND attribute_id = '" . (int)$product_attribute['attribute_id'] . "' AND language_id = '" . (int)$language_id . "' AND text = '" . $this->db->escape($product_attribute_description['text']) . "'");
                                                }
                                            }
                                        }
                                    }
                                ...
                                case "de_a": // Remove SELECTED attributes (and all its values)
                                    if (!empty($this->request->post['product_attribute'])) {
                                        foreach ($this->request->post['product_attribute'] as $product_attribute) {
                                            if ($product_attribute['attribute_id']) {
                                                $this->db->query("DELETE FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "' AND attribute_id = '" . (int)$product_attribute['attribute_id'] . "'");
                                ...
                                case "de_val": // Just remove old values
                                    $this->db->query("UPDATE " . DB_PREFIX . "product_attribute SET text = '' WHERE product_id = '" . (int)$product_id . "'");
                                    break;
        ...
        if (isset($this->request->post['lista_prod']) OR isset($this->request->post['mass_update']) OR isset($this->request->post['load_product_attr'])) { /// data filters
...
" . $plus_join . $plus_where;
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/masspattrupd', 'token=' . $this->session->data['token'], true)
        );
        ...
        $data['action'] = $this->url->link('extension/module/masspattrupd', 'token=' . $this->session->data['token'], true);

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/module/masspattrupd.tpl', $data));
    }
