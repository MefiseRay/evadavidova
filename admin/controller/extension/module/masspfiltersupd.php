<?php 
class ControllerExtensionModulemasspfiltersupd extends Controller { 
	private $error = array();
	
	public function index() {

    		$this->load->language('extension/module/masspfiltersupd');

		$this->document->addStyle('view/template/extension/module/masspfiltersupd.css');
		...
		// filters updates
		$data['masstxt_p_filters_updates'] = $this->language->get('masstxt_p_filters_updates');
		$data['masstxt_new_filters'] = $this->language->get('masstxt_new_filters');
		...
		if (isset($this->request->post['filters_ids_upd'])) {
			$data['filters_ids_upd'] = $this->request->post['filters_ids_upd'];
		} else {
			$data['filters_ids_upd'] = array();
		}
...
if ($this->user->hasPermission('modify', 'extension/module/masspfiltersupd')) { /// modify permision
...
switch ($this->request->post['upd_mode']) { /// update mode
    case "ad": // keep old filter and add new
		if (!empty($this->request->post['filters_ids_upd'])) {
			foreach ($this->request->post['filters_ids_upd'] as $filter_id) {
    ...
    case "re": // remove old filter and add new
	...
	case "de": // remove selected filters
		if (!empty($this->request->post['filters_ids_upd'])) {
			foreach ($this->request->post['filters_ids_upd'] as $filter_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_filter WHERE product_id = '" . (int)$product_id . "' AND filter_id = '" . (int)$filter_id . "'");
			}
...
$this->session->data['success'] = $this->language->get('masstxt_succes_products_filtered');
		...
		if (isset($this->session->data['success'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/masspfiltersupd', 'token=' . $this->session->data['token'], true)
   		);
		...
		$data['action'] = $this->url->link('extension/module/masspfiltersupd', 'token=' . $this->session->data['token'], true);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		
		$this->response->setOutput($this->load->view('extension/module/masspfiltersupd.tpl', $data));
	}
