...
class ControllerExtensionModulemassplayoutupd extends Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        $this->load->language('extension/module/massplayoutupd');

        $this->document->addStyle('view/template/extension/module/massplayoutupd.css');
        ...
        if (isset($this->request->post['mass_update'])) { /// update button
            ...
            if ($this->user->hasPermission('modify', 'extension/module/massplayoutupd')) { /// modify permision

                if (isset($this->request->post['selected'])) { /// avem produse selectate

                    if ($this->request->post['product_layout']) { /// avem update
                        ...
                        foreach ($this->request->post['selected'] as $product_id) { /// scanare produse
                            ...
                            if (isset($this->request->post['product_layout'])) {
                                ...
                                foreach ($this->request->post['product_layout'] as $store_id => $layout) {
                                    if ($layout['layout_id']) {
...
" . $plus_join . $plus_where;
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/massplayoutupd', 'token=' . $this->session->data['token'], true)
        );
        ...
        $data['action'] = $this->url->link('extension/module/massplayoutupd', 'token=' . $this->session->data['token'], true);

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/module/massplayoutupd.tpl', $data));
    }
