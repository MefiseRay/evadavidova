<?php 
class ControllerExtensionModulemasspnameupd extends Controller { 
	private $error = array();
	
	public function index() {

    		$this->load->language('extension/module/masspnameupd');
		
		$this->document->addStyle('view/template/extension/module/masspnameupd.css');
		...
		// name updates
		$data['masstxt_p_name_updates'] = $this->language->get('masstxt_p_name_updates');
		$data['masstxt_find_name'] = $this->language->get('masstxt_find_name');
		$data['masstxt_find_name_help'] = $this->language->get('masstxt_find_name_help');
		$data['masstxt_replace_name'] = $this->language->get('masstxt_replace_name');
		$data['masstxt_replace_name_help'] = $this->language->get('masstxt_replace_name_help');
		...
		if (isset($this->request->post['name_find'])) {
			$data['name_find'] = $this->request->post['name_find'];
		} else {
			$data['name_find'] = '';
		}
		if (isset($this->request->post['name_replace'])) {
			$data['name_replace'] = $this->request->post['name_replace'];
		} else {
			$data['name_replace'] = '';
		}
...
if ($this->user->hasPermission('modify', 'extension/module/masspnameupd')) { /// modify permision
...
switch ($this->request->post['upd_mode']) { /// update mode
	...
	if ($this->request->post['name_find'] != "") {
	  $this->db->query("UPDATE " . DB_PREFIX . "product_description SET name = REPLACE (name, 
	  '".$this->db->escape($this->request->post['name_find'])."', 
	  '".$this->db->escape($this->request->post['name_replace'])."') 
	...
	if ($this->request->post['name_replace'] != "") {
	  $this->db->query("UPDATE " . DB_PREFIX . "product_description SET name = CONCAT (
	  '".$this->db->escape($this->request->post['name_replace'])."', name) 
	  WHERE product_id = '" . (int)$product_id . "'".$lang_sql);
	  $count_updated_prod = $count_updated_prod + $this->db->countAffected();
	}
    break;
	...
	if ($this->request->post['name_replace'] != "") {
	  $this->db->query("UPDATE " . DB_PREFIX . "product_description SET name = CONCAT (
	  name, '".$this->db->escape($this->request->post['name_replace'])."') 
	  WHERE product_id = '" . (int)$product_id . "'".$lang_sql);
	  $count_updated_prod = $count_updated_prod + $this->db->countAffected();
	}
    break;
	...
	if ($this->request->post['name_replace'] != "") {
	  $this->db->query("UPDATE " . DB_PREFIX . "product_description SET name = '".$this->db->escape($this->request->post['name_replace'])."' 
	  WHERE product_id = '" . (int)$product_id . "'".$lang_sql);
	  $count_updated_prod = $count_updated_prod + $this->db->countAffected();
	}
    break;
...
$this->session->data['success'] = $this->language->get('masstxt_succes_products_filtered');
		...
		if (isset($this->session->data['success'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/masspnameupd', 'token=' . $this->session->data['token'], true)
   		);
		...
		$data['action'] = $this->url->link('extension/module/masspnameupd', 'token=' . $this->session->data['token'], true);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		
		$this->response->setOutput($this->load->view('extension/module/masspnameupd.tpl', $data));
	}
