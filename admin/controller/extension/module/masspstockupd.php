<?php 
class ControllerExtensionModulemasspstockupd extends Controller { 
	private $error = array();
	
	public function index() {

    		$this->load->language('extension/module/masspstockupd');
		
		$this->document->addStyle('view/template/extension/module/masspstockupd.css');
		...
		$data['masstxt_p_updates_quantity'] = $this->language->get('masstxt_p_updates_quantity');
		$data['masstxt_p_updates_value'] = $this->language->get('masstxt_p_updates_value');
		$data['masstxt_no_update_quantity_help'] = $this->language->get('masstxt_no_update_quantity_help');
		$data['masstxt_p_updates_minimum_quantity'] = $this->language->get('masstxt_p_updates_minimum_quantity');
		$data['masstxt_minimum_new_value'] = $this->language->get('masstxt_minimum_new_value');
		$data['masstxt_no_update_minimum_quantity_help'] = $this->language->get('masstxt_no_update_minimum_quantity_help');
		$data['masstxt_p_updates_subtract_stock'] = $this->language->get('masstxt_p_updates_subtract_stock');
		$data['masstxt_no_update_subtract_stock_help'] = $this->language->get('masstxt_no_update_subtract_stock_help');
		$data['masstxt_p_updates_out_of_stock_status'] = $this->language->get('masstxt_p_updates_out_of_stock_status');
		$data['masstxt_no_update_out_of_stock_status_help'] = $this->language->get('masstxt_no_update_out_of_stock_status_help');
		$data['masstxt_p_updates_requires_shipping'] = $this->language->get('masstxt_p_updates_requires_shipping');
		$data['masstxt_no_update_requires_shipping_help'] = $this->language->get('masstxt_no_update_requires_shipping_help');
		...
		$data['masstxt_upd_quantity_addition_help'] = $this->language->get('masstxt_upd_quantity_addition_help');
		$data['masstxt_upd_quantity_subtraction_help'] = $this->language->get('masstxt_upd_quantity_subtraction_help');
		$data['masstxt_upd_quantity_multiplication_help'] = $this->language->get('masstxt_upd_quantity_multiplication_help');
		$data['masstxt_upd_quantity_replacement_help'] = $this->language->get('masstxt_upd_quantity_replacement_help');
		...
		if (isset($this->request->post['quantity_upd'])) {
      			$data['quantity_upd'] = $this->request->post['quantity_upd'];
		} else {
      			$data['quantity_upd'] = '';
    		}

		if (isset($this->request->post['minimum_upd'])) {
      			$data['minimum_upd'] = $this->request->post['minimum_upd'];
		} else {
      			$data['minimum_upd'] = '';
    		}

		if (isset($this->request->post['subtract_upd'])) {
      			$data['subtract_upd'] = $this->request->post['subtract_upd'];
		} else {
      			$data['subtract_upd'] = 'no-update';
    		}

		if (isset($this->request->post['stock_status_id_upd'])) {
      			$data['stock_status_id_upd'] = $this->request->post['stock_status_id_upd'];
		} else {
      			$data['stock_status_id_upd'] = 'no-update';
    		}

		if (isset($this->request->post['shipping_upd'])) {
      			$data['shipping_upd'] = $this->request->post['shipping_upd'];
		} else {
      			$data['shipping_upd'] = 'no-update';
    		}
...
if ($this->user->hasPermission('modify', 'extension/module/masspstockupd')) { /// modify permision

if (isset($this->request->post['selected'])) { /// avem produse selectate

if ($this->request->post['quantity_upd']!="" OR $this->request->post['minimum_upd']!="" OR $this->request->post['subtract_upd']!="no-update" OR $this->request->post['stock_status_id_upd']!="no-update" OR $this->request->post['shipping_upd']!="no-update" OR $this->request->post['date_av_upd']!="" OR $this->request->post['status_upd']!="no-update") { /// avem update
...
if ($this->request->post['quantity_upd']!="") { /// avem quantity update
switch ($this->request->post['upd_mode']) {
    case "ad":
        if ($plus_set=="") { $prfx=" SET "; } else { $prfx=", "; }
        $plus_set.=$prfx."p.quantity = (p.quantity + '".(int)$this->request->post['quantity_upd']."')";
        break;
    case "su":
        if ($plus_set=="") { $prfx=" SET "; } else { $prfx=", "; }
        $plus_set.=$prfx."p.quantity = (p.quantity - '".(int)$this->request->post['quantity_upd']."')";
    ...
    case "mu":
        if ($plus_set=="") { $prfx=" SET "; } else { $prfx=", "; }
        $plus_set.=$prfx."p.quantity = (p.quantity * '".(float)$this->request->post['quantity_upd']."')";
        break;
    case "re":
        if ($plus_set=="") { $prfx=" SET "; } else { $prfx=", "; }
        $plus_set.=$prfx."p.quantity = '".(int)$this->request->post['quantity_upd']."'";
        break;
	}
}

if ($this->request->post['minimum_upd']!="") { /// avem minimum quantity update
    if ($plus_set=="") { $prfx=" SET "; } else { $prfx=", "; }
    $plus_set.=$prfx."p.minimum = '".(int)$this->request->post['minimum_upd']."'";
}

if ($this->request->post['subtract_upd']!="no-update") { /// avem subtract stock update
    if ($plus_set=="") { $prfx=" SET "; } else { $prfx=", "; }
    $plus_set.=$prfx."p.subtract = '".(int)$this->request->post['subtract_upd']."'";
}

if ($this->request->post['stock_status_id_upd']!="no-update") { /// avem Out Of Stock Status update
    if ($plus_set=="") { $prfx=" SET "; } else { $prfx=", "; }
    $plus_set.=$prfx."p.stock_status_id = '".(int)$this->request->post['stock_status_id_upd']."'";
}

if ($this->request->post['shipping_upd']!="no-update") { /// avem Requires Shipping update
    if ($plus_set=="") { $prfx=" SET "; } else { $prfx=", "; }
    $plus_set.=$prfx."p.shipping = '".(int)$this->request->post['shipping_upd']."'";
}
...
$this->session->data['success'] = $this->language->get('masstxt_succes_products_filtered');
		...
		if (isset($this->session->data['success'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/masspstockupd', 'token=' . $this->session->data['token'], true)
   		);
		...
		$data['action'] = $this->url->link('extension/module/masspstockupd', 'token=' . $this->session->data['token'], true);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		
		$this->response->setOutput($this->load->view('extension/module/masspstockupd.tpl', $data));
	}
