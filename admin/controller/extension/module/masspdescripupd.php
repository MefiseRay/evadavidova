<?php 
class ControllerExtensionModulemasspdescripupd extends Controller { 
	private $error = array();
	
	public function index() {

    		$this->load->language('extension/module/masspdescripupd');
		
		$this->document->addStyle('view/template/extension/module/masspdescripupd.css');
		...
		// description updates
		$data['masstxt_p_description_updates'] = $this->language->get('masstxt_p_description_updates');
		$data['masstxt_find_description'] = $this->language->get('masstxt_find_description');
		$data['masstxt_find_description_help'] = $this->language->get('masstxt_find_description_help');
		$data['masstxt_replace_description'] = $this->language->get('masstxt_replace_description');
		$data['masstxt_replace_description_help'] = $this->language->get('masstxt_replace_description_help');
		
		$data['masstxt_upd_lang'] = $this->language->get('masstxt_upd_lang');
		$data['masstxt_all_languages'] = $this->language->get('masstxt_all_languages');
		
		$data['masstxt_update_mode'] = $this->language->get('masstxt_update_mode');
		$data['masstxt_upd_mode_firep'] = $this->language->get('masstxt_upd_mode_firep');
		$data['masstxt_upd_mode_adbeg'] = $this->language->get('masstxt_upd_mode_adbeg');
		$data['masstxt_upd_mode_adend'] = $this->language->get('masstxt_upd_mode_adend');
		$data['masstxt_upd_mode_delad'] = $this->language->get('masstxt_upd_mode_delad');
		...
		if (isset($this->request->post['description_find'])) {
			$data['description_find'] = $this->request->post['description_find'];
		} else {
			$data['description_find'] = '';
		}
		if (isset($this->request->post['description_replace'])) {
			$data['description_replace'] = $this->request->post['description_replace'];
		} else {
			$data['description_replace'] = '';
		}

    		if (isset($this->request->post['upd_lang'])) {
      			$data['upd_lang'] = $this->request->post['upd_lang'];
		} else {
      			$data['upd_lang'] = 'all';
    		}
    		...
    		if (isset($this->request->post['upd_mode'])) {
      			...
      			$data['upd_mode'] = 'firep'; // find and replace
    		}
...
if ($this->user->hasPermission('modify', 'extension/module/masspdescripupd')) { /// modify permision
...
$lang_sql = "";
if ($this->request->post['upd_lang'] != 'all') {
	$lang_sql = " AND language_id = '" . (int)$this->request->post['upd_lang'] . "'";
}
...
switch ($this->request->post['upd_mode']) { /// update mode
    ...
    case "firep": // find and replace
	if ($this->request->post['description_find'] != "") {
	  $this->db->query("UPDATE " . DB_PREFIX . "product_description SET description = REPLACE (description, 
	  '".$this->db->escape($this->request->post['description_find'])."', 
	  '".$this->db->escape($this->request->post['description_replace'])."') 
	  WHERE product_id = '" . (int)$product_id . "'".$lang_sql);
	  $count_updated_prod = $count_updated_prod + $this->db->countAffected();
	}
    break;

    case "adbeg": // add to the begining
	if ($this->request->post['description_replace'] != "") {
	  $this->db->query("UPDATE " . DB_PREFIX . "product_description SET description = CONCAT (
	  '".$this->db->escape($this->request->post['description_replace'])."', description) 
	  WHERE product_id = '" . (int)$product_id . "'".$lang_sql);
	  $count_updated_prod = $count_updated_prod + $this->db->countAffected();
	}
    break;
   
    case "adend": // add to the end
	if ($this->request->post['description_replace'] != "") {
	  $this->db->query("UPDATE " . DB_PREFIX . "product_description SET description = CONCAT (
	  description, '".$this->db->escape($this->request->post['description_replace'])."') 
	  WHERE product_id = '" . (int)$product_id . "'".$lang_sql);
	  $count_updated_prod = $count_updated_prod + $this->db->countAffected();
	}
    break;

    case "delad": // delete and add
	if ($this->request->post['description_replace'] != "") {
	  $this->db->query("UPDATE " . DB_PREFIX . "product_description SET description = '".$this->db->escape($this->request->post['description_replace'])."' 
	  WHERE product_id = '" . (int)$product_id . "'".$lang_sql);
	  $count_updated_prod = $count_updated_prod + $this->db->countAffected();
	}
    break;
...
$this->session->data['success'] = $this->language->get('masstxt_succes_mass_update').$count_updated_prod.$this->language->get('masstxt_succes_x_products_updated');
...
$this->session->data['success'] = $this->language->get('masstxt_succes_products_filtered');
		...
		if (isset($this->session->data['success'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/masspdescripupd', 'token=' . $this->session->data['token'], true)
   		);
		...
		$data['action'] = $this->url->link('extension/module/masspdescripupd', 'token=' . $this->session->data['token'], true);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		
		$this->response->setOutput($this->load->view('extension/module/masspdescripupd.tpl', $data));
	}
