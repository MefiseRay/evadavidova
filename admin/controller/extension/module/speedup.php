...
class ControllerExtensionModuleSpeedup extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(empty($this->request->post['speedup_compression'])){
				$this->request->post['speedup_compression'] = 9;
			}
			if(empty($this->request->post['speedup_cache_expire_time'])){
				$this->request->post['speedup_cache_expire_time'] = 14400;
			}
			if(isset($this->request->post['speedup_compression'])){
				$compressionLevel = $this->request->post['speedup_compression'];
				$configcompress = $this->config->get('speedup_compression');
				$path = str_replace('admin/', 'config.php', DIR_APPLICATION);
				$content = file_get_contents($path);
				if(strpos($content, "define('CACHE_COMPRESSION', {$configcompress});")){
					$replace = str_replace("define('CACHE_COMPRESSION', {$configcompress});", "define('CACHE_COMPRESSION', {$compressionLevel});", $content);
				}else{
					$replace = $content . "define('CACHE_COMPRESSION', {$compressionLevel});";
				}
				file_put_contents($path, $replace);
			}
			if(isset($this->request->post['speedup_cache_expire_time'])){
				$configexpiretime = $this->config->get('speedup_cache_expire_time');
				$expiretime = $this->request->post['speedup_cache_expire_time'];
				$path = DIR_APPLICATION .'config.php';
				$content = file_get_contents($path);
				if(strpos($content, "define('CACHE_EXPIRE', {$configexpiretime});")){
					$replace = str_replace("define('CACHE_EXPIRE', {$configexpiretime});", "define('CACHE_EXPIRE', {$expiretime});", $content);
				}else{
					$replace = $content . "define('CACHE_EXPIRE', {$expiretime});";
				}
				file_put_contents($path, $replace);
				$path2 = str_replace('admin/', 'config.php', DIR_APPLICATION);
				$content2 = file_get_contents($path2);
				if(strpos($content2, "define('CACHE_EXPIRE', {$configexpiretime});")){
					$replace2 = str_replace("define('CACHE_EXPIRE', {$configexpiretime});", "define('CACHE_EXPIRE', {$expiretime});", $content2);
				}else{
					$replace2 = $content2 . "define('CACHE_EXPIRE', {$expiretime});";
				}
				file_put_contents($path2, $replace2);
			}

			$this->db->query("UPDATE " . DB_PREFIX . "setting SET `value` = '".(int)$this->request->post['speedup_product_count']."' WHERE `key` = 'config_product_count'");
			$this->db->query("UPDATE " . DB_PREFIX . "setting SET `value` = '".(int)$this->request->post['speedup_compression']."' WHERE `key` = 'config_compression'");
			$timezone = $this->request->post['speedup_timezone'];
			...
			$this->request->post['speedup_database_time_zone'] = $offset;

			$this->model_setting_setting->editSetting('speedup', $this->request->post);
			$this->session->data['success'] = $this->language->get('text_success');
			if(isset($this->request->get['stay']))
				$this->response->redirect($this->url->link('extension/module/speedup', 'token=' . $this->session->data['token'], true));
			$this->response->redirect($this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=module', true));
		}

		if ($this->config->get('speedup_cache_expire_time')) {
			$data['speedup_cache_expire_time'] = $this->config->get('speedup_cache_expire_time');
		} else {
			$data['speedup_cache_expire_time'] =  14400;
		}
		...
		if ($this->config->get('speedup_image_optimise_on') === 'on') {
		...
		if ($this->config->get('speedup_existing_image_compression_level')) {
			$data['image_crusher_existing_image_compression_level'] = $this->config->get('speedup_existing_image_compression_level');
		} else {
			$data['image_crusher_existing_image_compression_level'] =  $this->language->get('image_crusher_existing_image_compression_level');
		}
		if ($this->config->get('speedup_compression_level')) {
			$data['image_crusher_default_compression_level'] = $this->config->get('speedup_compression_level');
		} else {
			$data['image_crusher_default_compression_level'] =  $this->language->get('image_crusher_default_compression_level');
		}
		if (isset($this->request->post['speedup_timezone'])) {
			$data['speedup_timezone'] = $this->request->post['speedup_timezone'];
		} else {
			$data['speedup_timezone'] = $this->config->get('speedup_timezone');
		}
		...
		$data['breadcrumbs'] = array(array(
			...
			'href' => $this->url->link('extension/module/speedup', 'token=' . $this->session->data['token'], true)
		));

		$data['cssjs'] = $this->getAllCssJs();
		$data['action'] = $this->url->link('extension/module/speedup', 'token=' . $this->session->data['token'] . '&type=module', true);
		$data['stay_action'] = $this->url->link('extension/module/speedup', 'token=' . $this->session->data['token'].'&stay=1', true);
		$data['clear_cache'] = $this->url->link('extension/module/speedup/clear_cache', 'token=' . $this->session->data['token'], true);
		$data['optimize_table'] = $this->url->link('extension/module/speedup/optimize_table', 'token=' . $this->session->data['token'], true);
		$data['compress_css_js'] = $this->url->link('extension/module/speedup/compress_css_js', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=module', true);
		$data['speedup_status'] = 0;
		$data['speedup_defer'] = $this->config->get('speedup_defer');

		$filed = array('speedup_status','speedup_notfound_page','speedup_minify_css','speedup_minify_js','speedup_minify_html','speedup_image_lazyload','speedup_image_dimensions','speedup_url_alias_cache','speedup_compression_level','speedup_compression','speedup_defer','speedup_existing_image_compression_level','speedup_db_cache','speedup_page_cache');
		if ($this->request->server['REQUEST_METHOD'] == 'POST')
			foreach ($filed as $v) $data[$v] = $this->request->post[$v];
		else foreach ($filed as $v) $data[$v] = $this->config->get($v);

		if (isset($this->request->post['speedup_test_api_key'])) {
			$data['speedup_test_api_key'] = $this->request->post['speedup_test_api_key'];
		} else {
			$data['speedup_test_api_key'] = $this->config->get('speedup_test_api_key');
		}

		if (isset($this->request->post['speedup_test_url'])) {
			$data['speedup_test_url'] = $this->request->post['speedup_test_url'];
		} else {
			$data['speedup_test_url'] = $this->config->get('speedup_test_url');
		}

		if (isset($this->request->post['speedup_test_resources'])) {
			$data['speedup_test_resources'] = $this->request->post['speedup_test_resources'];
		} else {
			$data['speedup_test_resources'] = $this->config->get('speedup_test_resources');
		}
		
		if (isset($this->request->post['speedup_test_screenshot'])) {
			$data['speedup_test_screenshot'] = $this->request->post['speedup_test_screenshot'];
		} else {
			$data['speedup_test_screenshot'] = $this->config->get('speedup_test_screenshot');
		}

		if (isset($this->request->post['speedup_test_strategy'])) {
			$data['speedup_test_strategy'] = $this->request->post['speedup_test_strategy'];
		} else {
			$data['speedup_test_strategy'] = $this->config->get('speedup_test_strategy');
		}
		...
		$data['speedup_product_count'] = $this->config->get('config_product_count');
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('extension/module/speedup.tpl', $data));
	}
	public function clear_cache() {
		$path =  realpath(DIR_APPLICATION.'../')."/cache/*";
		$files = glob($path);
	...
	public function resizeImage($SrcImage,$DestImage,$Quality){
		$ex = pathinfo($SrcImage, PATHINFO_EXTENSION);
		if($SrcImage == 'png'){
	...
	public function install() {
		$path = str_replace('admin/', 'config.php', DIR_APPLICATION);
		$content = file_get_contents($path);
		$replace = $content . "define('CACHE_COMPRESSION', 9);define('CACHE_EXPIRE', 14400);";
		file_put_contents($path, $replace);

		$path2 = DIR_APPLICATION .'config.php';
		$content2 = file_get_contents($path2);
		$replace2 = $content2 . "define('CACHE_EXPIRE', 14400);";
		file_put_contents($path2, $replace2);

		$data = array(
			'speedup_compression'		=>	9,
			'speedup_cache_expire_time'	=>	14400,
		);
		$this->load->model('setting/setting');
		$this->model_setting_setting->editSetting('speedup', $data);
    ...
    public function uninstall() {
        $this->disable(true);
		$configexpiretime = $this->config->get('speedup_cache_expire_time');
        $compressionLevel = $this->config->get('speedup_compression');

        $path = str_replace('admin/', 'config.php', DIR_APPLICATION);
        $content = file_get_contents($path);
        $content = str_replace("define('CACHE_COMPRESSION', {$compressionLevel});", "", $content);
        $content = str_replace("define('CACHE_EXPIRE', {$configexpiretime});", "", $content);
        file_put_contents($path, $content);

		$path2 = DIR_APPLICATION .'config.php';
		$content2 = file_get_contents($path2);
		$content2 = str_replace("define('CACHE_EXPIRE', {$configexpiretime});", "", $content2);
