...
class ControllerExtensionModulemasspstoresupd extends Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        $this->load->language('extension/module/masspstoresupd');

        $this->document->addStyle('view/template/extension/module/masspstoresupd.css');
        ...
        // stores updates
        $data['masstxt_p_stores_updates'] = $this->language->get('masstxt_p_stores_updates');
        $data['masstxt_new_stores'] = $this->language->get('masstxt_new_stores');
        ...
        if (isset($this->request->post['stores_ids_upd'])) {
            $data['stores_ids_upd'] = $this->request->post['stores_ids_upd'];
        } else {
            $data['stores_ids_upd'] = array();
        }
        ...
        if (isset($this->request->post['mass_update'])) { /// update button
            ...
            if ($this->user->hasPermission('modify', 'extension/module/masspstoresupd')) { /// modify permision
                ...
                if (isset($this->request->post['selected'])) { /// avem produse selectate
                    ...
                    foreach ($this->request->post['selected'] as $product_id) { /// scanare produse
                        ...
                        switch ($this->request->post['upd_mode']) { /// update mode
                            case "ad": // keep old store and add new
                                if (!empty($this->request->post['stores_ids_upd'])) {
                                    foreach ($this->request->post['stores_ids_upd'] as $store_id) {
                            ...
                            case "re": // remove old store and add new
                            ...
                            case "de": // remove selected stores
                                if (!empty($this->request->post['stores_ids_upd'])) {
                                    foreach ($this->request->post['stores_ids_upd'] as $store_id) {
                                        $this->db->query("DELETE FROM " . DB_PREFIX . "product_to_store WHERE product_id = '" . (int)$product_id . "' AND store_id = '" . (int)$store_id . "'");
                                    }
...
" . $plus_join . $plus_where;
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/masspstoresupd', 'token=' . $this->session->data['token'], true)
        );
        ...
        $data['action'] = $this->url->link('extension/module/masspstoresupd', 'token=' . $this->session->data['token'], true);

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/module/masspstoresupd.tpl', $data));
    }
