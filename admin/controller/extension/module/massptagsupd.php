<?php 
class ControllerExtensionModulemassptagsupd extends Controller { 
	private $error = array();
	
	public function index() {

    		$this->load->language('extension/module/massptagsupd');
		
		$this->document->addStyle('view/template/extension/module/massptagsupd.css');
		...
		// tags updates
		$data['masstxt_p_tags_updates'] = $this->language->get('masstxt_p_tags_updates');
		$data['masstxt_new_tags'] = $this->language->get('masstxt_new_tags');
		$data['masstxt_new_tags_help'] = $this->language->get('masstxt_new_tags_help');
		...
		if (isset($this->request->post['tags_upd'])) {
			$data['tags_upd'] = $this->request->post['tags_upd'];
		} else {
			$tags_upd = array();
			foreach ($languages as $language) { $tags_upd[$language['language_id']] = ''; }
			$data['tags_upd'] = $tags_upd;
		}
...
if ($this->user->hasPermission('modify', 'extension/module/massptagsupd')) { /// modify permision
...
switch ($this->request->post['upd_mode']) { /// update mode
    ...
    case "ad": // keep old tags and add new
	foreach ($this->request->post['tags_upd'] as $language_id => $tags) {
		if ($tags!="") {
		$this->db->query("UPDATE " . DB_PREFIX . "product_description SET tag = CONCAT(tag,', ','" . $this->db->escape($tags) . "') WHERE product_id = '" . (int)$product_id . "' AND language_id = '" . (int)$language_id . "'");
		}
	}	
        break;
    
    case "re": // remove old tags and add new
	foreach ($this->request->post['tags_upd'] as $language_id => $tags) {
	...
	case "de": // remove tags for update from selected products
	foreach ($this->request->post['tags_upd'] as $language_id => $tags) {
		if ($tags!="") {
		$query_old_tags = $this->db->query("SELECT tag FROM " . DB_PREFIX . "product_description WHERE product_id = '" . (int)$product_id . "' AND language_id = '" . (int)$language_id . "'");
		$old_tags_tmp = array();
		$old_tags = array();
		$old_tags_tmp = explode(',', $query_old_tags->row['tag']);
		foreach ($old_tags_tmp as $old_tag) { $old_tags[]=trim($old_tag); }
		$upd_tags_tmp = array();
		$upd_tags = array();
		$upd_tags_tmp = explode(',', $this->db->escape($tags) );
		foreach ($upd_tags_tmp as $upd_tag) { $upd_tags[]=trim($upd_tag); }
		$new_tags = implode(',', array_diff($old_tags, $upd_tags));
		$this->db->query("UPDATE " . DB_PREFIX . "product_description SET tag = '" . $this->db->escape($new_tags) . "' WHERE product_id = '" . (int)$product_id . "' AND language_id = '" . (int)$language_id . "'");
		}
	}
        break;
...
$this->session->data['success'] = $this->language->get('masstxt_succes_products_filtered');
		...
		if (isset($this->session->data['success'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/massptagsupd', 'token=' . $this->session->data['token'], true)
   		);
		...
		$data['action'] = $this->url->link('extension/module/massptagsupd', 'token=' . $this->session->data['token'], true);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		
		$this->response->setOutput($this->load->view('extension/module/massptagsupd.tpl', $data));
	}
