...
class ControllerExtensionModuleOCdevSmcmStatic extends Controller
{
    ...
    public function index()
    {
        ...
        $data = array_merge($data, $this->language->load('extension/module/' . self::$_module_name));
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('extension/module/' . self::$_module_name, 'token=' . $this->session->data['token'], 'SSL')
            );
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('extension/module/' . self::$_module_name, 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
            );
        }

        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('extension/module/' . self::$_module_name, 'token=' . $this->session->data['token'], 'SSL');
        } else {
            $data['action'] = $this->url->link('extension/module/' . self::$_module_name, 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
        }
        ...
        $this->response->setOutput($this->load->view('extension/module/' . self::$_module_name . '.tpl', $data));
    }
