...
class ControllerExtensionPaymentWebmoneyWME extends Controller
{
    const MAX_LAST_LOG_LINES = 500;
    const FILE_NAME_LOG = 'webmoney_wme.log';
    private $error = array();
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/webmoney_wme', 'token=' . $this->session->data['token'], 'SSL')
        );

        $data['action'] = $this->url->link('extension/payment/webmoney_wme', 'token=' . $this->session->data['token'], 'SSL');
        $data['cancel'] = $this->url->link('extension/extension', 'type=payment&token=' . $this->session->data['token'], 'SSL');
        $data['clear_log'] = str_replace('&amp;', '&', $this->url->link('extension/payment/webmoney_wme/clearLog', 'token=' . $this->session->data['token'], 'SSL'));
        ...
        // Номер магазина
        if (isset($this->request->post['webmoney_wme_merch_r'])) {
            $data['webmoney_wme_merch_r'] = $this->request->post['webmoney_wme_merch_r'];
        } else {
            $data['webmoney_wme_merch_r'] = $this->config->get('webmoney_wme_merch_r');
        }

        // zp_merhant_key
        if (isset($this->request->post['webmoney_wme_secret_key'])) {
            $data['webmoney_wme_secret_key'] = $this->request->post['webmoney_wme_secret_key'];
        } else {
            $data['webmoney_wme_secret_key'] = $this->config->get('webmoney_wme_secret_key');
        }

        // zp_merhant_key X20
        if (isset($this->request->post['webmoney_wme_secret_key_x20'])) {
            $data['webmoney_wme_secret_key_x20'] = $this->request->post['webmoney_wme_secret_key_x20'];
        } else {
            $data['webmoney_wme_secret_key_x20'] = $this->config->get('webmoney_wme_secret_key_x20');
        }
        ...
        $data['webmoney_wme_fail_url'] = $server . 'index.php?route=extension/payment/webmoney_wme/fail';


        if (isset($this->request->post['webmoney_wme_order_confirm_status_id'])) {
            $data['webmoney_wme_order_confirm_status_id'] = $this->request->post['webmoney_wme_order_confirm_status_id'];
        } else {
            $data['webmoney_wme_order_confirm_status_id'] = $this->config->get('webmoney_wme_order_confirm_status_id');
        }

        if (isset($this->request->post['webmoney_wme_order_status_id'])) {
            $data['webmoney_wme_order_status_id'] = $this->request->post['webmoney_wme_order_status_id'];
        } else {
            $data['webmoney_wme_order_status_id'] = $this->config->get('webmoney_wme_order_status_id');
        }

        if (isset($this->request->post['webmoney_wme_order_fail_status_id'])) {
            $data['webmoney_wme_order_fail_status_id'] = $this->request->post['webmoney_wme_order_fail_status_id'];
        } else {
            $data['webmoney_wme_order_fail_status_id'] = $this->config->get('webmoney_wme_order_fail_status_id');
        }

        if (isset($this->request->post['webmoney_wme_hide_mode'])) {
            $data['webmoney_wme_hide_mode'] = $this->request->post['webmoney_wme_hide_mode'];
        } else {
            $data['webmoney_wme_hide_mode'] = $this->config->get('webmoney_wme_hide_mode');
        }

        if (isset($this->request->post['webmoney_wme_minimal_order'])) {
            $data['webmoney_wme_minimal_order'] = $this->request->post['webmoney_wme_minimal_order'];
        } else {
            $data['webmoney_wme_minimal_order'] = $this->config->get('webmoney_wme_minimal_order');
        }

        if (isset($this->request->post['webmoney_wme_maximal_order'])) {
            $data['webmoney_wme_maximal_order'] = $this->request->post['webmoney_wme_maximal_order'];
        } else {
            $data['webmoney_wme_maximal_order'] = $this->config->get('webmoney_wme_maximal_order');
        }
        ...
        if (isset($this->request->post['webmoney_wme_geo_zone_id'])) {
            $data['webmoney_wme_geo_zone_id'] = $this->request->post['webmoney_wme_geo_zone_id'];
        } else {
            $data['webmoney_wme_geo_zone_id'] = $this->config->get('webmoney_wme_geo_zone_id');
        }
        ...
        if (isset($this->request->post['webmoney_wme_status'])) {
            $data['webmoney_wme_status'] = $this->request->post['webmoney_wme_status'];
        } else {
            $data['webmoney_wme_status'] = $this->config->get('webmoney_wme_status');
        }

        if (isset($this->request->post['webmoney_wme_sort_order'])) {
            $data['webmoney_wme_sort_order'] = $this->request->post['webmoney_wme_sort_order'];
        } else {
            $data['webmoney_wme_sort_order'] = $this->config->get('webmoney_wme_sort_order');
        }

        if (isset($this->request->post['webmoney_wme_log'])) {
            $data['webmoney_wme_log'] = $this->request->post['webmoney_wme_log'];
        } else {
            $data['webmoney_wme_log'] = $this->config->get('webmoney_wme_log');
        }
        ...
        $this->response->setOutput($this->load->view('extension/payment/webmoney_wme.tpl', $data));
    }
    ...
    protected function validate()
    {
        ...
        if (!$this->request->post['webmoney_wme_merch_r']) {
        ...
        if (!$this->request->post['webmoney_wme_secret_key_x20']) {
    ...
    {
        return $this->user->hasPermission('modify', 'extension/payment/webmoney_wme');
    }
