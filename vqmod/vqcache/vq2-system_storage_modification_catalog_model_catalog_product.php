...
// << Related Options / Связанные опции 
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			...
			$sql .= " ORDER BY p.viewed DESC, p.sort_order";
		}
...
$query = $this->db->query($sql);
    ...
    public function getNewProducts($attribute_id, $limit) {

        $sql = ("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "product_attribute pa ON (p.product_id = pa.product_id) WHERE p.status = '1' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND pa.attribute_id = '" . $attribute_id . "' AND  LCASE(pa.text) = 'да' ORDER BY p.sort_order DESC LIMIT " . (int)$limit);
...
$query = $this->db->query($sql);
	...
	public function getProductRelatedByCategory($product_id) {
        ...
        $sql = ("SELECT * FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "product p ON (p2c.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p2c.category_id IN (SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "') AND p.product_id != '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY RAND() LIMIT 0, 8");
