...
class ControllerStartupSeoPro extends Controller {
	...
	private function validate() {
		...
		if (rawurldecode($url) != rawurldecode($seo) && strpos($url,($this->config->get('mfilter_url_param')?$this->config->get('mfilter_url_param'):'mfp').'=')===false && isset($this->request->server['SERVER_PROTOCOL'])) {
			header($this->request->server['SERVER_PROTOCOL'] . ' 301 Moved Permanently');
