...
class ControllerProductProduct extends Controller {
	...
	public function index() {
      ...
      // << Related Options / Связанные опции  
		...
		if ($product_info) {
            ...
            $this->document->addScript('/catalog/view/theme/logancee/js/jquery.jsticky.min.js');
              $this->document->addScript('/catalog/view/theme/logancee/js/stickyfloat.js');
             $this->document->addScript('/catalog/view/theme/logancee/js/hc-sticky.js');
	...
	$data['continuar'] = $this->language->get('continuar');
			...
			if ($this->customer->isLogged()) {
				...
				if (isset($product_id) && count($wishlist_products)) {
					foreach ($wishlist_products as $wishlist_product) {
						if ($product_id == $wishlist_product['product_id']) {
							$data['wishlist'] = true;
						}
					}
				}
      ...
      'product_option_id' => $result['product_option_id'][0],
      'product_option_value_id' => $result['product_option_value_id'][0],
      // >> Product Option Image PRO module
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
                        ...
                        $option_value_thumb = $option_value['thumb'] ? $this->model_tool_image->resize($option_value['thumb'], 50, 50) : '';
                        
                        if (!$option_value_thumb) {
                            $option_value_thumb = $option_value['image'] ? $this->model_tool_image->resize($option_value['image'], 50, 50) : '';
                        } 
                        
                        if (!$option_value_thumb && $option['name'] == 'Цвет') {
                            $option_value_thumb = $this->model_tool_image->resize('no_image.jpg', 50, 50);
                        }
      ...
      // >> Product Option Image PRO module
			...
			$data['review_total'] = $this->model_catalog_review->getTotalReviewsByProductId($this->request->get['product_id']);
    ...
    public function getProductOptionValueImages() {
        $this->load->model('tool/image');
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$json = $this->theme_options->getProductOptionValueImages($this->request->get['product_id'], $this->request->get['product_option_value_id']);
            
            if (isset($json['main_image']) && $json['main_image']) {
			     $json['main_image'] = $this->model_tool_image->resize($json['main_image'], $this->config->get($this->config->get('config_theme') . '_image_product_width'), $this->config->get($this->config->get('config_theme') . '_image_product_height'));
            } 
            
            if (isset($json['additional_image']) &&  $json['additional_image']) {
			     $json['additional_image'] = $this->model_tool_image->resize($json['additional_image'], $this->config->get($this->config->get('config_theme') . '_image_product_width'), $this->config->get($this->config->get('config_theme') . '_image_product_height'));
            } 
	...
	public function review() {
      ...
      // << Related Options / Связанные опции  
		...
		$results = $this->model_catalog_review->getReviewsByProductId($this->request->get['product_id'], ($page - 1) * 5, 5, $sort, $order);
