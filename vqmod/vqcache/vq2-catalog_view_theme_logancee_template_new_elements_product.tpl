...
$product_option_values = $theme_options->getProductOptionById($product['product_id'],1);
?>
...
<div class="product clearfix <?php if($theme_options->get( 'hover_product' ) != '0') { echo 'product-hover'; } ?>">
	<div class="left">
		<?php if ($product['thumb']) { ?>
			...
			<div class="image <?php if($theme_options->get( 'product_image_effect' ) == '1') { echo 'image-swap-effect'; } ?>">
				...
				<div class="quickview">
					<a href="index.php?route=product/quickview&pid=<?php echo $product['product_id']; ?>" title="<?php echo $product['name']; ?>"><?php if($theme_options->get( 'quickview_text', $config->get( 'config_language_id' ) ) != '') { echo $theme_options->get( 'quickview_text', $config->get( 'config_language_id' ) ); } else { echo 'Quickview'; } ?></a>
				</div>
				<?php } ?>
				...
				<a class="product-thumb" href="<?php echo $product['href']; ?>">
					...
					<img src="image/catalog/blank.gif" data-echo="<?php echo $product['thumb']; ?>" <?php /* Product Option Image PRO module << */ if (!isset($current_class)) $current_class = time(); ?> data-poip_id="image_<?php echo "".$current_class."_".$product['product_id']; ?>" <?php   /*  >> Product Option Image PRO module */ ?>  alt="<?php echo $product['name']; ?>" class="main-image <?php if($theme_options->get( 'product_image_effect' ) == '2') { echo 'zoom-image-effect'; } ?>" />
					<?php } else { ?>
					<img src="<?php echo $product['thumb']; ?>" <?php /* Product Option Image PRO module << */ if (!isset($current_class)) $current_class = time(); ?> data-poip_id="image_<?php echo "".$current_class."_".$product['product_id']; ?>" <?php   /*  >> Product Option Image PRO module */ ?>  alt="<?php echo $product['name']; ?>" class="main-image <?php if($theme_options->get( 'product_image_effect' ) == '2') { echo 'zoom-image-effect'; } ?>" />
					<?php } ?>
				</a>
			</div>
		<?php } else { ?>
			<div class="image">
				<?php if($theme_options->get( 'quick_view' ) == 1) { ?>
				<div class="quickview">
					<a href="index.php?route=product/quickview&pid=<?php echo $product['product_id']; ?>" title="<?php echo $product['name']; ?>"><i aria-hidden="true" class="arrow_expand"></i></a>
				</div>
				<?php } ?>
	...
	<div class="right">
		...
		<?php if ($product_option_values) { ?>
			<div class="product-color-swatches-container">				
				<ul class="mk-color-swatches">
					<?php foreach ($product_option_values as $option_value) { ?>
						<li>
							<div class="swatch-link<?php echo ($option_value['default_value'] ? ' selected' : ''); ?>" data-pid="<?php echo $product['product_id']; ?>" data-povid="<?php echo $option_value['product_option_value_id']; ?>">
								<img src="<?php echo $option_value['image']; ?>" alt="<?php echo $option_value['name']; ?>" title="<?php echo $option_value['name']; ?>" class="swatch-image" />	
							</label>
						</li>
					<?php } ?>
				</ul>
