		...
		if (isset($data['product_attribute'])) {
			foreach ($data['product_attribute'] as $product_attribute) {
				if ($product_attribute['attribute_id']) {
					...
					if (count($product_attribute['preset_id']) >= 1 && $this->config->get('atpresets_installed')==1) {
						//retreive preset in order to check it it maches the saved data
						$this->load->model('atpresets/atpresets');
						$are_same = true;

						$preset = array();
						
						$presets_text = $this->model_atpresets_atpresets->getManyPresetsTexts($product_attribute['preset_id']);
						...
						if (!empty($preset)) {
							if (count($product_attribute['preset_id']) == 1) //only single selected presets are checked
								foreach ( $product_attribute['product_attribute_description'] as $language_id => $product_attribute_description) {									
									if (html_entity_decode($product_attribute_description['text'], ENT_QUOTES, 'UTF-8') != html_entity_decode($preset[$language_id], ENT_QUOTES, 'UTF-8'))
										$are_same = false;									
								}
						} else {
							$are_same = false;
				...
				if ($this->config->get('atpresets_installed')==1) {
					...
					if ($are_same) { 
						foreach ($product_attribute['preset_id'] as $preset_id) {
							if ($preset_id>0)
								$this->db->query("INSERT INTO " . DB_PREFIX . "product_attribute SET product_id = '" . (int)$product_id . "', attribute_id = '" . (int)$product_attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', preset_id = '".$preset_id."', text = '" .  $this->db->escape($presets_text[$preset_id][$language_id]) . "'");
						}						
					} else {
		...
		foreach ($product_attribute_query->rows as $product_attribute) {
			...
			$product_attribute_description_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "' AND attribute_id = '" . (int)$product_attribute['attribute_id'] . "'");
				...
				if ($this->config->get('atpresets_installed')==1) {
					...
					$preset_id = array();
					$sql = "SELECT distinct(preset_id) FROM " . DB_PREFIX . "product_attribute WHERE attribute_id = '". (int)$product_attribute['attribute_id'] ."' and product_id='".(int)$product_id."'";								
					$query = $this->db->query($sql);
				...
				} else $preset_id = array();
				
			foreach ($product_attribute_description_query->rows as $product_attribute_description) {

				if ($this->config->get('atpresets_installed')==1) {
					if (isset($preset[$product_attribute_description['language_id']])) 
						$product_attribute_description['text'] = $preset[$product_attribute_description['language_id']]; 
				} 
