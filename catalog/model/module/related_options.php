...
class ModelModuleRelatedOptions extends Model
{
    ...
    public function getROCombSelectedByDefault($product_id, $search_request = '')
    {
        ...
        if ($search_request && !empty($ro_settings['spec_model'])) {
            ...
            $data['ros_to_select'] = $this->model_module_related_options->getRelatedOptionsIdsAutoSelectFirst($product_id);
        }
    }
    ...
    public function updateOrderProductAdditionalFields($product, $order_product_id)
    {
        ...
        if ($this->installed()) {
            ...
            $ro_adds = array(	'model'=>''
                                             ...
                                             ,'upc'=>$current_product_info['upc']
                                             ,'ean'=>$current_product_info['ean']
                                             ,'location'=>$current_product_info['location']
                                             );
            $ro_weight = $current_product_info['weight'];
                            ...
                            if ( $ro_comb[$ro_field_name] ) {
                                ...
                                if ($ro_field_name == 'model') {
                                    ...
                                    if ( $ro_settings['spec_'.$ro_field_name] == 1 ) {
                                        ...
                                        if ($ro_field_value == '') {
                                            $ro_field_value = $current_product_info[$ro_field_name];
            ...
            if (!empty($ro_settings['spec_weight'])) {
                $this->db->query("UPDATE " . DB_PREFIX . "order_product SET `weight`='".(float)($ro_weight*$quantity)."' WHERE order_product_id = " . (int)$order_product_id . "");
            }
            */
        }
    }
