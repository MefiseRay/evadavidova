...
class ModelExtensionDAjaxFilterManufacturer extends Model {
    ...
    public function getManufacturers($filter_data){
        ...
        $sql = "SELECT m.* FROM `".DB_PREFIX."af_temporary` tp LEFT JOIN `".DB_PREFIX."manufacturer` m ON m.manufacturer_id = tp.manufacturer_id WHERE tp.manufacturer_id != '0' GROUP BY tp.manufacturer_id ORDER BY m.sort_order ASC ";
    ...
    public function getTotalQuery($manufacturers, $table_name){
        $sql = $table_name.".manufacturer_id IN (".implode(',', $manufacturers[0]).")";
        return $sql;
    }
}