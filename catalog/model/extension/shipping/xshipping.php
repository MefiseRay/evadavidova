        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            if ($status) {
                ...
                $quote_data['xshipping' . $i] = array(
                    ...
                    'description' => $this->config->get('xshipping_description' . $i),
                    'cost' => ($shipping_cost ? $shipping_cost : ''),
                    'tax_class_id' => $this->config->get('xshipping_tax_class_id' . $i),
                    'text' => ($shipping_cost ? $this->currency->format($this->tax->calculate($shipping_cost, $this->config->get('xshipping_tax_class_id' . $i), $this->config->get('config_tax')), $currency_code) : '')
                );
