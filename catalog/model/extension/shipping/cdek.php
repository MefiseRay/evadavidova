...
class ModelExtensionShippingCdek extends Model {
	...
	function getQuote($address) 
	{
		...
		if(!$address['city'] && $address['zone']) 
		{
			...
			$cdekZoneData = $this->db->query("SELECT cityName FROM `" . DB_PREFIX . "cdek_city` WHERE (`regionName` LIKE '%".$zoneNameParts[0]."%' OR `cityName` LIKE '%".$zoneNameParts[0]."%') AND `center` = '1' LIMIT 1")->row;
			if($cdekZoneData) {
		...
		if ($to_data && $address['city']) {
			...
			if (is_array($cdek_cities) && isset($cdek_cities['geonames']) && count($cdek_cities['geonames'])) {
				...
				{
					$sql = "SELECT regionName FROM  `" . DB_PREFIX . "cdek_city` WHERE `cityName` = '".$address['zone']."'";
	...
	private function getPVZList($city_id, $weight = 0) {
		...
		$pvz_list_data = $this->getURL($this->getInfo()->getBaseUrl().'pvzlist.php?cityid=' . $city_id.'&type=ALL', new parser_xml());
