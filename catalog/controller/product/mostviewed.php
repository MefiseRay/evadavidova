...
class ControllerProductMostViewed extends Controller { 	
	...
	public function index() { 
		...
		if((int)$this->config->get('coc_mostviewed_status') == 1) {
			...
			$coc_mostviewed_description = $this->config->get('coc_mostviewed_description');
			
			$this->document->setTitle($coc_mostviewed_description[(int)$this->config->get('config_language_id')]['meta_title']);
			
			$this->document->setDescription($coc_mostviewed_description[(int)$this->config->get('config_language_id')]['meta_description']);
			
			$this->document->setKeywords($coc_mostviewed_description[(int)$this->config->get('config_language_id')]['meta_keyword']);
			
			$this->document->addLink($this->url->link('product/mostviewed', ''), 'canonical');
			...
			$data['breadcrumbs'][] = array(
				...
				'text' => $coc_mostviewed_description[(int)$this->config->get('config_language_id')]['heading_title'],
				
				'href' => $this->url->link('product/mostviewed', $url)
				
			);
	
			$data['heading_title'] = $coc_mostviewed_description[(int)$this->config->get('config_language_id')]['heading_title'];
			...
			if((int)$this->config->get('coc_mostviewed_quantity') == 0) {
				
				$total_limit = 99999;
				
			}else{
	
				$total_limit = (int)$this->config->get('coc_mostviewed_quantity'); //define the total products to get
			...
			$results = $this->model_coc_product001->getMostViewedProducts($total_limit);
			...
			$data['sorts'][] = array(
				...
				'href'  => $this->url->link('product/mostviewed', 'sort=p.price&order=ASC' . $url)
			...
			$data['sorts'][] = array(
				...
				'href'  => $this->url->link('product/mostviewed', 'sort=p.price&order=DESC' . $url)
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/mostviewed.tpl')) {
				...
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/product/mostviewed.tpl', $data));
				
			} else {
				
				$this->response->setOutput($this->load->view('default/template/product/mostviewed.tpl', $data));
