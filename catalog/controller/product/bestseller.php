...
class ControllerProductBestseller extends Controller { 	
	...
	public function index() { 
		...
		if((int)$this->config->get('coc_bestseller_status') == 1) {
			...
			$coc_bestseller_description = $this->config->get('coc_bestseller_description');
			
			$this->document->setTitle($coc_bestseller_description[(int)$this->config->get('config_language_id')]['meta_title']);
			
			$this->document->setDescription($coc_bestseller_description[(int)$this->config->get('config_language_id')]['meta_description']);
			
			$this->document->setKeywords($coc_bestseller_description[(int)$this->config->get('config_language_id')]['meta_keyword']);
			
			$this->document->addLink($this->url->link('product/bestseller', ''), 'canonical');
			...
			$data['breadcrumbs'][] = array(
				...
				'text' => $coc_bestseller_description[(int)$this->config->get('config_language_id')]['heading_title'],
				
				'href' => $this->url->link('product/bestseller', $url)
				
			);
	
			$data['heading_title'] = $coc_bestseller_description[(int)$this->config->get('config_language_id')]['heading_title'];
			...
			if((int)$this->config->get('coc_bestseller_quantity') == 0) {
				
				$total_limit = 99999;
				
			}else{
	
				$total_limit = (int)$this->config->get('coc_bestseller_quantity'); //define the total products to get
			...
			$results = $this->model_coc_product->getBestSellerProducts($total_limit);
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/bestseller.tpl')) {
				...
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/product/bestseller.tpl', $data));
				
			} else {
				
				$this->response->setOutput($this->load->view('default/template/product/bestseller.tpl', $data));
