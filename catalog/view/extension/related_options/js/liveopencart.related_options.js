					...
					if (inst.hide_inaccessible) {
						...
						if ( typeof(inst.custom_radioEnableDisable) != 'undefined' && inst.custom_radioEnableDisable( $(radio_options[i]), option_value_disabled ) ) {
							...
							if (option_value_disabled) {
								...
								$(radio_options[i]).parent().parent().css("display", "none");
							} else {
								$(radio_options[i]).parent().fadeTo("fast", 1);
								$(radio_options[i]).parent().parent().css("display", "inline-block");
							}
					...
					if ( product_option_value_id ) {
						...
						if (inst.hide_inaccessible) {
							...
							if ( option_value_disabled ) {
								...
								$option_value_container.parent().css("display", "none");
							} else {
								$option_value_container.fadeTo("fast", 1);
								$option_value_container.parent().css("display", "inline-block");
							}
						}
					}
				});
