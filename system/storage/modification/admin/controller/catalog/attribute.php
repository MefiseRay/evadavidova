...
class ControllerCatalogAttribute extends Controller {
	...
	protected function getForm() {
		$data['heading_title'] = $this->language->get('heading_title');
			...
			$data['text_preset_values'] = $this->language->get('text_preset_values');	
			$data['text_preset_value'] = $this->language->get('text_preset_value');
			$data['entry_value'] = $this->language->get('entry_value');	
			$data['text_add_preset'] = $this->language->get('text_add_preset');
		...
		if ($this->config->get('atpresets_installed')==1) {
			if (isset($this->request->get['attribute_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
				...
				$all_presets = $this->model_atpresets_atpresets->getPresets(array('attribute_id' => $this->request->get['attribute_id']));			
				$presets = array();
				foreach ($all_presets as $preset) {
					$presets[] = $this->model_atpresets_atpresets->getPreset($preset['preset_id']);	
				}
			}

			if (isset($this->request->post['presets'])) {
				$data['presets'] = $this->request->post['presets'];
	...
	protected function validateForm() {
		...
		if ($this->config->get('atpresets_installed')==1) {
			if (isset($this->request->post['presets'])) {
				foreach ($this->request->post['presets'] as $key => $preset) {
					if (isset($preset['descriptions'])) {
						$all_null = true;
						foreach ($preset['descriptions'] as $desc) {
							if ($desc != '') {
								$all_null = false;
							}
						}
					} else $all_null = false;
					if ($all_null) unset($this->request->post['presets'][$key]['text']);
				}
			}
		}
