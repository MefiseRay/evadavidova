...
class ThemeOptions {
  	...
  	public function getDataProduct($product_id) {
  		...
  		if($result) {
  			...
  			$output = array(
  				...
  				'date_end' => $date_end,
  				'is_latest' => $this->isLatestProduct($product_id)
  			);
  		}	
    ...
    public function numberOf($numberof, $value, $suffix) {
        $numberof = abs($numberof);
        $keys = array(2, 0, 1, 1, 1, 2);
        $mod = $numberof % 100;
        $suffix_key = $mod > 4 && $mod < 20 ? 2 : $keys[min($mod%10, 5)];

        return $numberof . ' ' . $value . $suffix[$suffix_key];
    }
    
    public function getProductAttributeText($product_id,$attribute_id) {
		$registry = $this->data['registry'];
		$config = $registry->get('config');
  		...
  		$query = $db->query("SELECT * FROM " . DB_PREFIX . "product_attribute WHERE attribute_id = '" . $attribute_id . "' AND product_id='" . $product_id . "' AND language_id = '" . $config->get('config_language_id') . "'");
  		 		
  		if ($query->num_rows) {
			$output = $query->row['text'];
		}
	...
	public function getProductOptionById($product_id,$option_id) {	
		...
		$model_image = $registry->get('model_tool_image');
  		...
  		$query = $db->query("SELECT pov.option_value_id, pov.thumb, pov.default_value, ov.image, ovd.name, pov.product_option_value_id FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (pov.option_value_id = ovd.option_value_id) WHERE pov.option_id = '" . $option_id . "' AND pov.product_id='" . $product_id . "' AND ovd.language_id = '" . $config->get('config_language_id') . "' AND pov.quantity > 0");
  		 		
  		if ($query->num_rows) {
			foreach ($query->rows as $result) {
                
                $image = $result['thumb'] ? $result['thumb'] : '';
                ...
                if (!$image) {
                    $query_poi = $db->query("SELECT * FROM " . DB_PREFIX . "poip_option_image WHERE product_id='" . $product_id . "' AND product_option_value_id = '" . $result['product_option_value_id'] . "' LIMIT 1");
                    if ($query_poi->num_rows) {
                        $image = $query_poi->row['image'];
                    } else {
                        //$image = 'no_image.png';
                    }
                }
				...
				$output[] = array(
					...
					'name' => $result['name'],
					'image' => $model_image->resize($image, 30, 30),
    ...
    public function getProductOptionValueImages($product_id, $product_option_value_id) {
        $registry = $this->data['registry'];
		$config = $registry->get('config');
		$model_image = $registry->get('model_tool_image');
        $width = $config->get($config->get('config_theme') . '_image_product_width');
        $height = $config->get($config->get('config_theme') . '_image_product_height');
        ...
        $query_poi = $db->query("SELECT * FROM " . DB_PREFIX . "poip_option_image WHERE product_id='" . $product_id . "' AND product_option_value_id = '" . $product_option_value_id . "' ORDER BY sort_order ASC LIMIT 2");

        if ($query_poi->num_rows) {
             $image = $query_poi->rows[0]['image'];
             $additional_image = (isset($query_poi->rows[1]['image']) ? $query_poi->rows[1]['image'] : $image);
