...
class info extends cdek_integrator {
	...
	public function getTariffList() {
		...
		return array(
			...
			'301' => array(
				'title'		=> 'До постомата InPost (Д-С)',
				'mode_id'	=> 2,
				'im'		=> 1
			),
			'302' => array(
				'title'		=> 'До постомата InPost (С-С)',
				'mode_id'	=> 4,
				'im'		=> 1
			)
		);
	...
	public function getCityByName($city) {
		...
		$response = $this->getURL($this->ajax_url . 'city/getListByTerm/json.php?q=' . urlencode($city) . '&name_startsWith=' . urlencode($city), new parser_json());
