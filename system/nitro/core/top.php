...
function serveBrowserCacheHeadersIfNecessary($filemtime)
{
    ...
    $cache_filename = generateNameOfCacheFile();
    $cookie_string = nitroGetLanguage() . nitroGetCurrency() . getSupportedCookiesPrefix();

    if (empty($session['NitroCookiesToPage'][$cache_filename])) {
        $session['NitroCookiesToPage'][$cache_filename] = $cookie_string;
        return;
    } else if ($session['NitroCookiesToPage'][$cache_filename] != $cookie_string) {
        $session['NitroCookiesToPage'][$cache_filename] = $cookie_string;
        return;
    }
