...
function optimizeJS($scripts, $excludes = array(), $excludes_meta = array()) {
    ...
    if (getNitroPersistence('Mini.JS')) {
        $scripts = minify_file('js', $scripts, $excludes, $excludes_meta);
    }
...
function minify_file($type, $files, $excludes, $excludes_meta = array()) {
    ...
    foreach ($files as $hash=>$file) {
        ...
        if (!is_excluded_file($file, $excludes, $excludes_meta)) {
            ...
            } else if (!$recache && file_exists($targetAbsolutePath)) {
                touch($targetAbsolutePath);// This fixes the issue where styles were disapearing after a while, because the cron was clearing the minified files, because there mtime was old, although they are still in use. Do not remove this.
            }
